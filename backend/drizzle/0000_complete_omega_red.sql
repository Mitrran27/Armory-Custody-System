...
CREATE TYPE "public"."system_role" AS ENUM('admin', 'duty_officer', 'armorer', 'auditor');--> statement-breakpoint
...
ALTER TABLE "qr_scanners" ADD CONSTRAINT "qr_scanners_door_id_doors_id_fk" FOREIGN KEY ("door_id") REFERENCES "public"."doors"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "qr_scanners_door_id_idx" ON "qr_scanners" USING btree ("door_id");--> statement-breakpoint
CREATE UNIQUE INDEX "rooms_zone_id_idx" ON "rooms" USING btree ("zone_id");--> statement-breakpoint
