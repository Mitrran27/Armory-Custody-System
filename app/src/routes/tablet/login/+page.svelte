<script lang="ts">
	...
	async function submitEmail(e: SubmitEvent) {
		...
		loading = true;
		try {
			const res = await auth.requestOtp(email.trim().toLowerCase());
	...
	async function submitCode(e: SubmitEvent) {
		...
		try {
			...
			if (!auth.hasRole('armorer', 'admin')) {
				error = 'This account is not an armorer. The tablet is for armory staff only.';
				auth.signOut();
				return;
			}
			goto('/tablet');
...
<svelte:head><title>Sign in — Armorer Tablet</title></svelte:head>
	...
	<div class="bracket-frame w-full max-w-sm rounded-sm border border-line bg-panel p-6">
		<div class="mb-6 flex items-center gap-2.5">
			...
			<div class="leading-tight">
				<p class="font-display text-sm font-semibold text-ink">Armorer Tablet</p>
				<p class="text-[10px] text-ink-dim">Stationed in Zone C — Armory</p>
			</div>
		</div>

		{#if step === 'email'}
			<p class="eyebrow mb-1">Sign in</p>
			<h1 class="mb-4 font-display text-lg font-medium text-ink">Enter your staff email</h1>
			<form onsubmit={submitEmail} class="space-y-3">
				<input
					type="email"
					required
					bind:value={email}
					placeholder="you@cre8iot.com"
					autocomplete="email"
